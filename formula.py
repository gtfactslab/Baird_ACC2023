...
class STLTree(STLFormula):
    ...
    def robustness(self, y, t, update_pre=False):
        if self.combination_type == "and":
            if self.pre_flag:
                print([formula.robustness(y,max(t+self.timesteps[i], 0)) for i, formula in enumerate(self.subformula_list)].append(self.pre))
                r = min( [formula.robustness(y,max(t+self.timesteps[i], 0)) for i, formula in enumerate(self.subformula_list)].append(self.pre) )
                if update_pre:
                    self.pre = r
                return r
            else:
            ...
            return max( [formula.robustness(y,max(t+self.timesteps[i], 0)) for i, formula in enumerate(self.subformula_list)] )
